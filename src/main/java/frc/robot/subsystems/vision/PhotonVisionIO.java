...
public class PhotonVisionIO implements VisionIO {
    ...
    public PhotonVisionIO(Drive drive) {
        ...
        photonEstimator = new PhotonPoseEstimator(FIELD_TAG_LAYOUT, ROBOT_TO_CAMERA);
    ...
    public void processVision() {
        ...
        for (PhotonPipelineResult cameraResult : camera.getAllUnreadResults()) {
            ...
            visionEst.ifPresent(
                    est -> {
                        drive.addVisionMeasurement(est.estimatedPose.toPose2d(), est.timestampSeconds, curStdDevs);
                    });
        }
    }
